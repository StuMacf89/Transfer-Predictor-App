...
if page == 'Transfer Performance Estimator': 
    ...
    if selected_player:
        ...
        average_performance = filtered_players['previous_performance_club_league_interaction'].mean()
        ...
        ranked_players = filtered_players.sort_values(by='previous_performance_club_league_interaction', ascending=False)
        ...
        columns_to_display = ['Player', 'main_position', 'previous_performance_club_league_interaction', 'Age', 'Comp_x', 'Team', 'Market value']
    ...
    norm = plt.Normalize(vmin=filtered_players['previous_performance_club_league_interaction'].min() / filtered_players['Market value'].max(),
                          vmax=filtered_players['previous_performance_club_league_interaction'].max() / filtered_players['Market value'].min())
    ...
    filtered_players['performance_to_value_ratio'] = (
        filtered_players['previous_performance_club_league_interaction'] / filtered_players['Market value']
    )
    ...
    scatter = ax.scatter(
        filtered_players['previous_performance_club_league_interaction'],
    ...
    ax.scatter(
        selected_player_data['previous_performance_club_league_interaction'],
    ...
    if filtered_distribution_data.empty:
        ...
        filtered_distribution_data = filtered_distribution_data.dropna(subset=['previous_performance_club_league_interaction'])
    ...
    if highlight_data.empty:
        ...
        selected_performance = highlight_data['previous_performance_club_league_interaction'].values[0]

        # Create the histogram
        fig, ax = plt.subplots(figsize=(10, 6))

        # Normalize the colors for the gradient effect
        norm = Normalize(vmin=filtered_distribution_data['previous_performance_club_league_interaction'].min(),
                         vmax=filtered_distribution_data['previous_performance_club_league_interaction'].max())
        cmap = plt.cm.RdYlGn  # Red-to-green color gradient

        # Create the histogram
        counts, bins, patches = ax.hist(
            filtered_distribution_data['previous_performance_club_league_interaction'],
            bins=30,
            color='grey',
            edgecolor='black'
        )
...
# Inside the page logic
    ...
    def calculate_boosted_index(df, features):
        ...
        for feature in features:
            ...
            threshold = np.percentile(df[feature].dropna(), 95)
            
            # Apply boosting for values above the threshold
            boosted_feature = np.where(
                df[feature] > threshold,
                1 / (1 + np.exp(-10 * (df[feature] - threshold) / threshold)),
                df[feature]  # Keep the original value if it's below the threshold
            )
            
            # Accumulate the boosted feature into the boosted index
            boosted_index += boosted_feature
        
        # Normalize the index (optional: to scale it between 0–1)
        boosted_index /= len(features)  # Average across all features
        boosted_index /= np.nanmax(boosted_index)  # Normalize to max value
    ...
    if selected_features:
        ...
        filtered_df = df_tech_radar[selected_features + ['Player', 'main_position', 'Age', 'Comp_x', 'Team','previous_club_strength_relative_to_league']].dropna()
        ...
        filtered_df['Position Specific Index'] = (filtered_df['Position Specific Index']/ filtered_df['previous_club_strength_relative_to_league'])
