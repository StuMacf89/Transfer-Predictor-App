...
# Inside the page logic
    ...
    def calculate_boosted_index(df, features):
        ...
        for feature in features:
            ...
            # Apply boosting only for values above the threshold
            boosted_feature = df[feature].copy()
            boosted_feature[df[feature] > threshold] = (
                1 / (1 + np.exp(-10 * (df[feature][df[feature] > threshold] - threshold) / threshold))
            )
            
            # Keep the original values for those below the threshold
    ...
    if selected_features:
        ...
        # Add filters for main_position and Age
