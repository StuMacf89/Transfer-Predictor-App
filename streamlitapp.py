...
if page == 'Transfer Performance Estimator': 
    ...
    def apply_non_linear_scaling(value, threshold=95):
        """
        Apply non-linear scaling to a single value based on a given threshold.
        If the value exceeds the threshold, apply a sigmoid-based boosting;
        otherwise, return the value as-is.
        """
        if value > threshold:
            # Apply sigmoid transformation for values above the threshold
            boosted_value = 1 / (1 + np.exp(-20 * (value - threshold) / threshold))  # Sigmoid boosting
    ...
    def calculate_boosted_index(df, features):
       ...
       for feature in features:
           ...
           # Calculate the boosted feature value
           boosted_feature = np.where(
               df[feature] > threshold,
               1 / (1 + np.exp(-20 * (df[feature] - threshold) / threshold)),  # Sigmoid boosting for values above threshold
               0  # No boost for values below or equal to the threshold
           )
           
           # Add the boosted feature to the original feature value
           combined_feature = df[feature] + boosted_feature
           
           # Clip values to a maximum of 1
           combined_feature = np.clip(combined_feature, 0, 1)
           
           # Accumulate the feature into the boosted index
